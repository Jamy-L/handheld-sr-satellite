...
def compute_sat_mask(image, exposure):
    imshape = image.shape
    sat_mask = cuda.device_array(imshape, DEFAULT_NUMPY_FLOAT_TYPE)
    ...
    cuda_compute_sat_mask[blockspergrid, threadsperblock](image, exposure, sat_mask)
    
    return sat_mask
    
@cuda.jit()
def cuda_compute_sat_mask(image, exposure, sat_mask):
    x, y = cuda.grid(2)
    imshape_y, imshape_x = image.shape
    
    if not (0 <= x < imshape_x and
            0 <= y < imshape_y):
        return
    
    if exposure * image[y, x] > 0.99:
        sat_mask[y, x] = 1
    else:
        sat_mask[y, x] = 0
...
def base_detail_decomp(cuda_img, sigma_bd=1):
    """
    extract base and details from a single numba image    

    Parameters
    ----------
    samples : device array [hy, hx]
    ...
    torch_im = th.as_tensor(cuda_img, dtype=DEFAULT_TORCH_FLOAT_TYPE, device="cuda")
    
    th_base, th_detail = base_detail_decomp_(torch_im[None, None], gaussian_filter) 
    th_detail = th_detail[0, 0]
            
    detail = cuda.as_cuda_array(th_detail)
    base = cuda.as_cuda_array(th_base)
    
    return base, detail
    
def base_detail_decomp_(samples, gaussian_filter):
...
def flowEstimation(samplesLR, ME, device, gaussian_filter , warping, sr_ratio = 2):
    ...
    return flow.reshape(b, num_im, 2, h, w)

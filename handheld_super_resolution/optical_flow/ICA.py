...
def init_ICA(ref_img, options, params, ref_sat_mask):
    ...
    compute_hessian[blockspergrid, threadsperblock](cuda_gradx, cuda_grady,
                                                    tile_size, hessian, ref_sat_mask)
...
@cuda.jit
def compute_hessian(gradx, grady, tile_size, hessian, sat_mask):
    imshape = gradx.shape
    n_patch_y, n_patch_x, _, _ = hessian.shape
    ...
    # Non existing patches
    if not (patch_idx < n_patch_x and
            patch_idy < n_patch_y):
        return
    ...
    # if (0 <= patch_pos_idx < imshape[1] and 
    #     0 <= patch_pos_idy < imshape[0]):
        
    #     hessian[patch_idy, patch_idx, 0, 0] = patch_pos_idx
    #     hessian[patch_idy, patch_idx, 0, 1] = patch_pos_idy
    ...
    for i in range(tile_size):
        for j in range(tile_size):
            ...
            if not (inbound and sat_mask[pixel_global_idy, pixel_global_idx] == 0):# valid pixel
                continue 
            # if not (inbound):# valid pixel
            #     continue 
            
            local_gradx = gradx[pixel_global_idy, pixel_global_idx]
            local_grady = grady[pixel_global_idy, pixel_global_idx]
            
            # hessian[patch_idy, patch_idx, 0, 0] = pixel_global_idx
            # hessian[patch_idy, patch_idx, 0, 1] = pixel_global_idy
    ...
    # Normalized hessian
    hessian[patch_idy, patch_idx, 0, 0] = local_hessian[0, 0]/n
    hessian[patch_idy, patch_idx, 0, 1] = local_hessian[0, 1]/n
    hessian[patch_idy, patch_idx, 1, 0] = local_hessian[1, 0]/n
    hessian[patch_idy, patch_idx, 1, 1] = local_hessian[1, 1]/n


def ICA_optical_flow(cuda_im_grey, cuda_ref_grey,
                     cuda_gradx, cuda_grady,
                     hessian, cuda_pre_alignment,
                     options, params, ref_sat_mask, sat_mask, debug = False):
    ...
    for iter_index in range(n_iter):
        ICA_optical_flow_iteration(
            ...
            options, params, iter_index, ref_sat_mask, sat_mask)
...
def ICA_optical_flow_iteration(ref_img, gradsx, gradsy, comp_img, alignment, hessian, options, params,
                               iter_index, ref_sat_mask, sat_mask):
    """
    ...
    ICA_get_new_flow[blockspergrid, threadsperblock](
        ref_img, comp_img,
        gradsx, gradsy,
        alignment, hessian, tile_size, ref_sat_mask, sat_mask)   
...
@cuda.jit
def ICA_get_new_flow(ref_img, comp_img, gradx, grady, alignment, hessian, tile_size, ref_sat_mask, sat_mask):
    """
    ...
    for i in range(tile_size):
        for j in range(tile_size):
            ...
            if ref_sat_mask[pixel_global_idy, pixel_global_idx] == 1 :
                continue
            ...
            if not (sat_mask[floor_y, floor_x] == 0 and
                    sat_mask[floor_y, ceil_x] == 0 and
                    sat_mask[ceil_y, floor_x] == 0 and
                    sat_mask[ceil_y, ceil_x] == 0):
                continue

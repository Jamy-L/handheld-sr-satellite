...
def upsample_alignments(referencePyramidLevel, alternatePyramidLevel, previousAlignments, upsamplingFactor, tileSize, previousTileSize):
    ...
    if tileSize > 32:
        cuda_upsample_alignments_big[blockspergrid, threadsperblock](
...
def local_search(referencePyramidLevel, alternatePyramidLevel,
    ...
    if distance == 'L1':
        if tileSize > 32:
            cuda_L1_local_search_big[blockspergrid, threadsperblock](referencePyramidLevel, alternatePyramidLevel,
                                                          tileSize, searchRadius,
                                                          upsampledAlignments)
        else:
        ...
        if tileSize > 32:
            cuda_L2_local_search_big[blockspergrid, threadsperblock](referencePyramidLevel, alternatePyramidLevel,
                                                          tileSize, searchRadius,
                                                          upsampledAlignments)
        else:
...
# ---- Band aid solution for Ts > 32


@cuda.jit
def cuda_upsample_alignments_big(referencePyramidLevel, alternatePyramidLevel, upsampledAlignments, previousAlignments, upsamplingFactor, tileSize, previousTileSize):
    subtile_x, subtile_y = cuda.grid(2)
    n_tiles_y_prev, n_tiles_x_prev, _ = previousAlignments.shape
    n_tiles_y_new, n_tiles_x_new, _ = upsampledAlignments.shape
    h, w = referencePyramidLevel.shape
    ...
    for i in range(tileSize):
        for j in range(tileSize):
            ...
            new_idy = idy + int(candidate_alignment_0_shift[1])
            new_idx = idx + int(candidate_alignment_0_shift[0])
            if (0 <= new_idx < w and
                0 <= new_idy < h):
                dist_ += abs(referencePyramidLevel[idy, idx] - alternatePyramidLevel[new_idy, new_idx])
            else:
                dist_ = 1/0
    if dist_ < dist:
        dist = dist_
...
@cuda.jit
def cuda_L1_local_search_big(referencePyramidLevel, alternatePyramidLevel,
                         tileSize, searchRadius, upsampledAlignments):
    n_patchs_y, n_patchs_x, _ = upsampledAlignments.shape
    h, w = alternatePyramidLevel.shape
    tile_x, tile_y = cuda.grid(2)
            ...
            for i in range(tileSize):
                for j in range(tileSize):
                    ...
                    if (0 <= idx < w and
                        ...
                        new_idx = idx + int(local_flow[0]) + search_shift_x
                        new_idy = idy + int(local_flow[1]) + search_shift_y
...
@cuda.jit
def cuda_L2_local_search_big(referencePyramidLevel, alternatePyramidLevel,
                         tileSize, searchRadius, upsampledAlignments):
    n_patchs_y, n_patchs_x, _ = upsampledAlignments.shape
    h, w = alternatePyramidLevel.shape
    tile_x, tile_y = cuda.grid(2)

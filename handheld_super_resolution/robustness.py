...
def init_robustness(ref_img, options, params):
    ...
    CFA_pattern = cuda.to_device(np.zeros((2, 2))) # The CFA array is not used in grey mode
...
def compute_robustness(comp_img, ref_local_stats,
                       ref_exposure, exposure,
                       flows, options, params):
    """
...
def apply_noise_model(d_p, ref_local_stats,
                      ref_exposure, exposure,
...
def cuda_apply_noise_model(d_p, ref_local_stats,
    ...
    for channel in range(n_channels):
        # Note : the code here uses the single exposure noise curves,,
        # and shifts the position on the curve based on the exposure ratio.
        # It would be preferable to generate a curve for each ratio and to use them.
        ref_brightness = ref_local_stats[idy, idx, 0, channel] * ref_exposure
        comp_brightness = ref_local_stats[idy, idx, 0, channel] * exposure 
        
        id_noise_comp = clamp(round(1000 * comp_brightness), 0, 1001) # id on the noise curve
        id_noise_ref = clamp(round(1000 * ref_brightness), 0, 1001)
        
        # temporary solution
        tmp = ref_brightness * exposure/ref_exposure
        tmp = clamp(round(1000 * ref_brightness), 0, 1001)
        
        d_t =  diff_curve[tmp]
        sigma_t = max(std_curve[id_noise_ref], std_curve[id_noise_comp])

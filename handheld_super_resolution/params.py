...
def check_params_validity(params, imshape):
    ...
    for lvl, (factor, ts) in enumerate(zip(params['block matching']['tuning']['factors'], params['block matching']['tuning']['tileSizes'])):
        ...
        if n_tiles_y < 2 or n_tiles_x < 2:
            # Clipping the pyramid height
            params['block matching']['tuning']['factors'] = params['block matching']['tuning']['factors'][:lvl] 
            params['block matching']['tuning']['tileSizes'] = params['block matching']['tuning']['tileSizes'][:lvl]
            params['block matching']['tuning']['searchRadia'] = params['block matching']['tuning']['searchRadia'][:lvl]
            params['block matching']['tuning']['distances'] = params['block matching']['tuning']['distances'][:lvl]
    
            
            
            warnings.warn(
                "Image of shape {} is incompatible with the given "\
                "block matching tile sizes and factors : at level {}, "\
                "coarse image of shape {} cannot be divided into "\
                "tiles of size {}. The pyramid has been clipped.".format(
                    imshape, lvl, (lvl_imshape_y, lvl_imshape_x), ts))
            break

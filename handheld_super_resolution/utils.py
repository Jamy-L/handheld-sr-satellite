...
def round_iso(ISO):
    """
    The ISO curves are ploted for ISOs that can be written
    ISO = 100 * 2**n
    For an ISO between 100 and 3200, this function finds n using bit
    manipulation.
    ...
    """
    ISO = round(ISO/100)
    if ISO == 1: # separate case because the bit string is shorter than 4 char
        return 100 
    
    strongest_bit_rank = ISO.bit_length()-1 # ISO/100 is rounded to 2**n_bits or 2**(n_bits-1)
    if bin(ISO)[3] == '0':
        return 100*(1 << strongest_bit_rank) # 2**n_bits
    
    else:
        return 100*(1 << (strongest_bit_rank+1)) # 2**(n_bits + 1)
